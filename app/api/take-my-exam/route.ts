...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query for take-my-exam page by id
    const query = { 
      id: "take-my-exam"
    };
    ...
    if (!content) {
      console.log('No content found for take-my-exam in pages collection');
      await client.close();
      return NextResponse.json({ error: 'Page not found' }, { status: 404, headers: corsHeaders });
    }
    
    console.log('Found take-my-exam content, id:', content.id, 'pageType:', content.pageType);
    await client.close();
